...
const buildValidateRequest = ({ token, body }) =>
  fetch(new URL("/promo-codes/validate", resolveApiBaseUrl()), {
    method: "POST",
    headers: {
...
export async function POST(request) {
  ...
  let response = await buildValidateRequest({ token, body });
  ...
  if (response.status === 401) {
    ...
    if (fallbackToken) {
      ...
      response = await buildValidateRequest({ token, body });
