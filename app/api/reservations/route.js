...
const buildReservationRequest = ({ token, body }) =>
  fetch(new URL("/reservations", resolveApiBaseUrl()), {
    method: "POST",
    headers: {
...
export async function POST(request) {
  ...
  let response = await buildReservationRequest({ token, body });
  ...
  if (response.status === 401) {
    ...
    if (fallbackToken) {
      ...
      response = await buildReservationRequest({ token, body });
