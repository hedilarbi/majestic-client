  ...
  const handleToggleSeat = useCallback(
    async (cell) => {
      ...
      if (action === "reserve" && fixedSeatMeta && !isPricingMetaAvailable(fixedSeatMeta)) {
        alert("Ce tarif est épuisé pour cette séance.");
        return;
      }

      if (action === "reserve" && !fixedSeatMeta && !hasAvailableVariablePricing) {
        alert("Aucun tarif disponible pour cette séance.");
        return;
      }
      ...
      try {
        ...
        if (status === 409) {
          errorMessage = "Ce siège vient d'être réservé. Veuillez réessayer.";
        ...
        if (!ok) {
          errorMessage = data?.message || "Impossible de réserver.";
        ...
        alert(errorMessage || "Une erreur est survenue. Merci de réessayer.");
