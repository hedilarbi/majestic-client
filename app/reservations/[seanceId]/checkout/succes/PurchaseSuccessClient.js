...
export default function PurchaseSuccessClient({ seanceId, bookingId }) {
  ...
  const subscriptionTransaction =
    booking?.subscriptionTransaction && typeof booking.subscriptionTransaction === "object"
  ...
  const isSubscriptionPayment =
    String(booking?.paymentMethod || "").toLowerCase() === "subscription" ||
    Boolean(subscriptionTransaction?.subscriptionCode);
  const remainingSubscriptionCredits = Number.isFinite(
    Number(subscriptionTransaction?.remainingCredits),
  )
    ? Number(subscriptionTransaction.remainingCredits)
    : null;
  const usedSubscriptionCredits = Number.isFinite(
    Number(subscriptionTransaction?.creditsUsed),
  )
    ? Number(subscriptionTransaction.creditsUsed)
    : 0;
  const ticketsHref = canOpenTickets
    ...
    : seanceInfo?.eventId
      ...
      : "/";
  const ticketsLabel = canOpenTickets ? "Consulter mes billets" : "Voir l'événement";
      ...
      <div className="relative mx-auto w-full max-w-6xl">
        <section className="mb-10 flex flex-col items-center text-center">
          ...
          <p className="mt-3 max-w-2xl text-sm text-white/60 md:text-base">
            Votre réservation est enregistrée. Retrouvez ci-dessous les détails
            de votre séance et votre code booking.
          </p>
        </section>
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <article className="rounded-3xl border border-white/10 bg-[#131b22]/80 p-6 shadow-2xl backdrop-blur-xl">
            ...
            <div className="mt-5 flex gap-4">
              <div className="relative h-36 w-24 shrink-0 overflow-hidden rounded-xl border border-white/10 bg-white/5">
                <Image
                  src={displayPoster}
                  alt={seanceInfo?.title || "Affiche événement"}
                  fill
          ...
          <article className="rounded-3xl border border-accent/30 bg-accent/5 p-6 shadow-2xl">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-lg font-bold text-white">
                      {remainingSubscriptionCredits === null
                        ? "Credits abonnement utilises"
                        : `${remainingSubscriptionCredits} crédit${
                            remainingSubscriptionCredits > 1 ? "s" : ""
                          } restant${
                            remainingSubscriptionCredits > 1 ? "s" : ""
                          }`}
                    </p>
                    <p className="mt-1 text-sm text-white/65">
                      {usedSubscriptionCredits > 0
                        ? `${usedSubscriptionCredits} crédit${
                            usedSubscriptionCredits > 1 ? "s" : ""
                          } utilisé${
                            usedSubscriptionCredits > 1 ? "s" : ""
                          } pour cet achat.`
                        : "Paiement regle via votre abonnement."}
                    </p>
                    {subscriptionTransaction?.subscriptionCode ? (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-lg font-bold text-white">Confirmation envoyée</p>
            ...
            <p className="mt-5 text-sm leading-relaxed text-white/65">
              Un email de confirmation contenant votre récapitulatif booking a
              ete envoyé. Conservez le code booking pour toute assistance.
            </p>
          </article>
        </section>
      </div>
